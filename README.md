...
- **A double-sided A4 cheat sheet** (`sac-cheatsheet.html`, linked from the SAC overview) condensing the whole section: the band scale and verb ladder, the linkage moves, the six controls, the three threat modelling principles, the ten vulnerabilities with the phrases that plant them, the criteria set, the APPs, the instrument chooser, the mitigation menu, sentence frames and a pre-submission checklist. Print-first — self-contained CSS so what prints does not depend on the site theme, and laid out to fill exactly two A4 sides.
